...
export function registerSegEditorPanel(shell: AppShell, opts: { live: LiveScene; store: LocalBlobStore; onStatus?: (s: string) => void }): void {
  ...
  function render() {
    ...
    const syncTool = () => { const cur = g().__segTool?.(); if (cur && cur.activeEffect) g().__setSegTool(segId, cur.activeEffect, { diameterMm: cur.diameterMm, sphere: cur.sphere, segment: active }); };
    $(".sl-seg-new").addEventListener("click", async () => { if (!segId) { await ensureSeg(); } else { active = g().__addSegment(segId); syncTool(); } render(); });
    root.querySelectorAll(".sl-seg-row").forEach((el) => el.addEventListener("click", (e) => { if ((e.target as HTMLElement).dataset.vis) return; active = Number((el as HTMLElement).dataset.seg); syncTool(); render(); }));
