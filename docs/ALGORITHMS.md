...
| **A-1o** ✅ | **Per-segment opacity + shading mode**: `attrTex` (rgba16float) carries per-segment `.r` = opacity (palette alpha) and `.g` = shading mode (a mode palette). The SDF `SegmentField` samples it (`bindingCount` 2) and branches: **surface** = crisp SDF shell, **volume** = DVR fill of the interior (translucent cloud). `SegmentationLogic.setLabelOpacity` / `setLabelShading`. | **DONE** — opaque surface occludes what's behind (red-over-blue 0), translucent reveals (46, `opacity.ts`); volume shading is see-through DVR (surface 0 vs volume 4333, `shading.ts`). Demo **Randomize look** (random opacity + surface/volume per segment) + **Reset look**. |
