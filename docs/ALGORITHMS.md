...
| **A-1o** ✅ | **Per-segment opacity** (translucent surface models): the palette alpha = per-segment opacity, baked (from the flooded region label, so non-zero across the whole shell) into a second `attrTex` (rgba16float `.r`) the SDF `SegmentField` samples (`bindingCount` 2). `SegmentationLogic.setLabelOpacity`. Opacity <1 → the DVR compositing shows inner segments through outer ones. | **DONE** — an opaque front segment fully occludes the one behind (red-over-blue 0) while a translucent one reveals it (46), `algorithms/test/opacity.ts`. Demo **Surfaces: Opaque/Translucent** toggle. |
