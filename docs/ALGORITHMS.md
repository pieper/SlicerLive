...
| **A-1m** ✅ | **Multi-label colour**: the colorized SDF stores per-label colour in the texture (rgba16float: rgb=palette colour, a=signed mm); JFA seed carries the region label; the alpha-only blur keeps colour seams crisp. `SegmentField.colorFromTexture`, `SegmentationLogic` palette + `setLabelColor`. Works in both sdf & surface modes. | **DONE** — one merged surface shows each label in its own colour with a crisp seam where neighbours meet (`algorithms/test/multi-label.ts`, 3 colours both modes). Demo Poke/Paint each add a new coloured label. |
