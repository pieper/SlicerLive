...
image = (
    ...
    .pip_install("vtk", "pynrrd", "numpy", "requests", "fastapi",
                 "cupy-cuda12x[ctk]", "PyNvVideoCodec", "zarr<3", "numcodecs")
    .run_commands(
...
class Renderer:
    ...
    def boot(self):
        ...
        self.pool.submit(self._prewarm)   # compile cupy kernels off the first-frame path

    def _prewarm(self):
        try:
            import cupy as cp
            a = cp.arange(64, dtype=cp.float32)
            _ = cp.clip(a * 0.5 + 1, 0, 255).astype(cp.uint8)
    ...
    def _ensure_loaded(self):
        ...
        self._vtk, self._np, self._n2v, self._cp, self._nvc = vtk, np, numpy_support, cp, nvc
        t0 = time.time()
    ...
    def make_encoder(self):
        return self._nvc.CreateEncoder(W, H, "NV12", True, codec="h264", bitrate=6000000,
    ...
    def render_h264(self, enc):
        ...
        rgba = self._n2v.vtk_to_numpy(a).reshape(H, W, 4)[::-1, :, :3]
        d = cp.asarray(rgba).astype(cp.float32)
        r, g, b = d[:, :, 0], d[:, :, 1], d[:, :, 2]
        y = cp.clip(0.257 * r + 0.504 * g + 0.098 * b + 16, 0, 255).astype(cp.uint8)
        u = cp.clip(-0.148 * r - 0.291 * g + 0.439 * b + 128, 0, 255).astype(cp.uint8)
        vv = cp.clip(0.439 * r - 0.368 * g - 0.071 * b + 128, 0, 255).astype(cp.uint8)
        uv = cp.empty((H // 2, W), cp.uint8); uv[:, 0::2] = u[::2, ::2]; uv[:, 1::2] = vv[::2, ::2]
        nv12 = cp.asnumpy(cp.ascontiguousarray(cp.concatenate([y, uv], 0)))
        bs = enc.Encode(nv12)
