...
impl Session {
    ...
    pub fn encode_picture<I: EncoderInput, O: EncoderOutput>(
        ...
        let mut encode_pic_params = NV_ENC_PIC_PARAMS {
            ...
            // LiveRenderer: force EVERY IDR-typed picture to a real KEY frame. Without this, NVENC
            // emits KEY only for frame 0 of a session and INTRA_ONLY_FRAME thereafter — which
            // WebCodecs (and any decoder that keyframe-gates) rejects. FORCEIDR makes each encoded
            // frame independently decodable, which the streaming transport requires.
            encodePicFlags: if params.picture_type
                == crate::sys::nvEncodeAPI::NV_ENC_PIC_TYPE::NV_ENC_PIC_TYPE_IDR
            {
                crate::sys::nvEncodeAPI::_NV_ENC_PIC_FLAGS::NV_ENC_PIC_FLAG_FORCEIDR as u32
                    | crate::sys::nvEncodeAPI::_NV_ENC_PIC_FLAGS::NV_ENC_PIC_FLAG_OUTPUT_SPSPPS as u32
            } else {
                0
            },
            ..Default::default()
